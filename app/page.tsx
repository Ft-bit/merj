    ...
    <div style={{ background: '#000', color: '#fff', fontFamily: '"SF Pro Display",-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', overflowX: 'hidden' }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes iconDrop{
          0%{opacity:0;transform:translateY(-200px) scale(0.2) rotate(-20deg)}
          60%{opacity:1;transform:translateY(12px) scale(1.04) rotate(1deg)}
          80%{transform:translateY(-5px) scale(0.97) rotate(-.5deg)}
        ...
        .icon-drop{opacity:0;transform:translateY(-200px) scale(0.2)}
        .icon-drop.in{animation:iconDrop .9s cubic-bezier(.34,1.56,.64,1) forwards}

        .text-slide{opacity:0;transform:translateX(-50px)}
        .text-slide.in{animation:textSlide .7s ease .35s forwards}
        .text-slide.in.rev{animation:textSlide .7s ease .35s forwards}
      ...
      <section style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'7rem 2rem 5rem', position:'relative', textAlign:'center', overflow:'hidden' }}>
          ...
          {PLATFORMS.map((p, i) => (
            <div key={p.id} style={{ display:'flex', alignItems:'flex-end' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'10px', opacity:heroIn?1:0, transition:`opacity .6s ease ${i*120+200}ms` }}>
                <div className="ic" style={{ width:'84px', height:'84px', background:p.bg, boxShadow:`0 20px 50px ${p.glow}55, inset 0 1px 0 rgba(255,255,255,.22)`, animationDelay:`${i*.3}s`, animationDuration:`${3.5+i*.3}s` }} onClick={() => router.push('/listings')}>
                  <div style={{ position:'relative', zIndex:4 }}>
                    {p.img
                      ? <Image src={p.img} alt={p.name} width={50} height={50} style={{ objectFit:'contain', filter:'drop-shadow(0 2px 8px rgba(0,0,0,.5))', display:'block' }} />
                      : <GlobeIcon size={46} />
                    }
                  </div>
                </div>
        ...
        <div style={{ position:'relative', zIndex:3, maxWidth:'860px', opacity:heroIn?1:0, transition:'opacity 1s ease .4s' }}>
          ...
          <h1 className="hh" style={{ fontSize:'clamp(2.8rem,7.5vw,6rem)', fontWeight:'900', letterSpacing:'-.04em', lineHeight:.96, marginBottom:'1.5rem', transition:'all .4s ease' }}>
            {buy ? <><span style={{ color:'rgba(255,255,255,.9)' }}>The place websites</span><br /><span style={{ color:'#e8c547' }}>change hands.</span></> : <><span style={{ color:'rgba(255,255,255,.9)' }}>Turn your website</span><br /><span style={{ color:'#e8c547' }}>into money.</span></>}
          </h1>
          <p className="hs" style={{ fontSize:'1.08rem', color:'rgba(255,255,255,.38)', lineHeight:1.8, margin:'0 auto 2.5rem', maxWidth:'520px', transition:'all .4s ease' }}>
            {buy ? 'Every listing on Merj is ownership-verified before you see it. Your money moves only when ownership does.' : 'List any live website in minutes. Merj verifies ownership, secures payment, and transfers everything automatically. You just get paid.'}
          </p>
        ...
        return (
          ...
          >
            ...
            <div className={rev ? 'pr rev' : 'pr'} style={{ maxWidth:'1080px', margin:'0 auto', display:'flex', alignItems:'center', flexDirection:rev?'row-reverse':'row', gap:'4.5rem', position:'relative', zIndex:1 }}>
              ...
              {/* ICON — drops in when section enters viewport */}
              <div style={{ flex:'0 0 auto', display:'flex', flexDirection:'column', alignItems:'center', gap:'1rem' }}>
                <div style={{ position:'relative' }}>
                  ...
                  <div className={`pc icon-drop${isIn?' in':''}`} style={{ width:'220px', height:'220px', background:p.bg, boxShadow:`0 32px 80px ${p.glow}44, inset 0 1px 0 rgba(255,255,255,.2)`, animationDelay: isIn ? `${i * 0.1}s` : '0s' }}>
                    <div style={{ position:'relative', zIndex:4 }}>
                      {p.img
                        ? <Image src={p.img} alt={p.name} width={120} height={120} style={{ objectFit:'contain', filter:'drop-shadow(0 4px 18px rgba(0,0,0,.5))', display:'block' }} />
                        : <GlobeIcon size={90} />
                      }
                    </div>
                  </div>
              ...
              {/* TEXT */}
              <div className={`text-slide${isIn?' in':''}${rev?' rev':''}`} style={{ flex:1 }}>
                ...
                <button className="pb" onClick={() => router.push(buy?'/listings':'/login')} style={{ background:p.id==='vercel'?'#fff':p.glow, color:'#000', boxShadow:`0 10px 28px ${p.glow}44` }}>
      ...
      <section style={{ padding:'9rem 2rem', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          <div style={{ fontSize:'.68rem', fontWeight:'700', letterSpacing:'.24em', color:'rgba(255,255,255,.2)', textTransform:'uppercase', marginBottom:'1.5rem', display:'flex', alignItems:'center', justifyContent:'center', gap:'14px' }}>
            <div style={{ width:'28px', height:'1px', background:'rgba(255,255,255,.2)' }} />Start today<div style={{ width:'28px', height:'1px', background:'rgba(255,255,255,.2)' }} />
          </div>
        ...
        <div style={{ maxWidth:'1080px', margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1.5rem' }}>
          ...
          <div style={{ display:'flex', gap:'1.75rem' }}>
            {['Marketplace','Sell','Sign in'].map(l => (
              <button key={l} onClick={() => router.push(l==='Marketplace'?'/listings':'/login')} style={{ background:'none', border:'none', color:'rgba(255,255,255,.18)', cursor:'pointer', fontSize:'.78rem', fontFamily:'inherit', padding:0, transition:'color .2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color='rgba(255,255,255,.6)' }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color='rgba(255,255,255,.18)' }}>{l}</button>
            ))}
          </div>
        </div>
      </footer>
